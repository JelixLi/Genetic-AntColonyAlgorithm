...
def get_best(get_fitness, targetLen, optimalFitness, geneSet, display,
             ...
             poolSize=1, crossover=None,init_parents=None):
    ...
    for improvement in _get_improvement(fnNewChild, fnGenerateParent,
                                        maxAge, poolSize,init_parents,get_fitness):
        display(improvement)
...
def _get_improvement(new_child, generate_parent, maxAge, poolSize,init_parents,get_fitness):
    if not init_parents==None:
        assert(poolSize>len(init_parents))
        parents = []
        historicalFitnesses = []
        bestParent = generate_parent()
        for genes in init_parents:
            parent = Chromosome(genes, get_fitness(genes), Strategies.Create)
            if parent.Fitness > bestParent.Fitness:
                yield parent
                bestParent = parent
                historicalFitnesses.append(parent.Fitness)
            parents.append(parent)
        residual_poolSize = poolSize - len(init_parents)
        for _ in range(residual_poolSize):
